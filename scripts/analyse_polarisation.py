...
path_dossier_sortie = "../out_scripts/lumiere_polarisee/lumiere_polarisee_FORTRAN_CUDA_" + nom_hdf
...
for iphi in xrange(0,NBPHI_fortran):
	for i in xrange(NTHV):
		fortran_pol[iphi][i] = math.sqrt(pow( tab_fortran['real_refl'][1, i-1, iphi, 0],2) +
...
if (NBPHI_cuda/2) == NBPHI_fortran:
	for iphi in xrange(0,NBPHI_cuda/2,5):
		...
		title('Comparaison lumiere polarisee Fortran - Cuda pour phi='+str(phi[iphi])+" deg")
		xlabel('Theta (deg)')
		...
		title("Rapport lumiere polarisee Fortran et Cuda pour phi="+str(phi[iphi])+" deg")
		xlabel("Theta (deg)")
		...
		title("Difference lumiere polarisee Fortran - Cuda pour phi="+str(phi[iphi])+" deg")
		xlabel("Theta (deg)")
