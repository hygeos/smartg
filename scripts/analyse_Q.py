...
path_dossier_sortie = "../out_scripts/Q/Q_FORTRAN_CUDA_" + nom_hdf
...
for iphi in xrange(0,NBPHI_fortran):
	for i in xrange(NTHV):
		fortran_pol[iphi][i] = tab_fortran['real_refl'][1, i-1, iphi, 0]
...
if (NBPHI_cuda/2) == NBPHI_fortran:
	for iphi in xrange(0,NBPHI_cuda/2,5):
		...
		title('Comparaison Q Fortran - Cuda pour phi='+str(phi[iphi])+" deg")
		xlabel('Theta (deg)')
		ylabel('Q')
		grid(True)
		...
		title("Rapport Q Fortran et Cuda pour phi="+str(phi[iphi])+" deg")
		xlabel("Theta (deg)")
		ylabel("Rapport Q")
		grid(True)
		...
		title("Difference Q Fortran - Cuda pour phi="+str(phi[iphi])+" deg")
		xlabel("Theta (deg)")
		ylabel("Difference Q")
		grid(True)
